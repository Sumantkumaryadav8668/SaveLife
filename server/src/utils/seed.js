...
import SupportTicket from '../modules/chatbot/support-ticket.model.js';
...
export const seedData = async (shouldExit = false) => {
  try {
    ...
    // Seed physical Entities (Hospitals, Police, Rescue teams)
    // Centered around coordinate Bangalore, India (Lng: 77.5946, Lat: 12.9716)
    const entities = [
      {
        ...
        hospitalResources: {
          roomsTotal: 40,
          roomsOccupied: 15,
          bedsGeneralTotal: 100,
          bedsGeneralOccupied: 45,
          bedsIcuTotal: 20,
          bedsIcuOccupied: 8,
          bloodBank: [
    ...
    const users = [
      ...
      {
        name: 'Regular Citizen User',
        email: 'user@rapidaid.com',
        password: 'userpassword',
        role: 'user',
        phone: '+918080808080',
        status: 'active',
        emergencyContacts: [
          { name: 'Contact Parent', phone: '+919000000001', relation: 'Parent' },
          { name: 'Contact Friend', phone: '+919000000002', relation: 'Friend' }
        ]
      },
      {
        name: 'Hospital Admin Manager',
        email: 'hospital@rapidaid.com',
      ...
      {
        name: 'Police Patrol Officer',
        email: 'police@rapidaid.com',
      ...
      {
        name: 'Rescue Chief Officer',
        email: 'rescue@rapidaid.com',
    ...
    // Seed some mock audit logs for role creation
    const sysAdminUser = await User.findOne({ email: 'admin@rapidaid.com' }).exec();
    await AuditLog.create({
      action: 'SYSTEM_INITIALIZATION',
      performedBy: sysAdminUser._id,
      details: 'RapidAid System Initialized with default seeder stations and operational admin users.'
    });
